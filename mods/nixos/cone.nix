...
in {
  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      {
        assertion = (builtins.elem "docker" cfg.supplementaryGroups -> config.virtualisation.docker.enable == true);
        message = "{option}`cfg.supplementaryGroups` contains the `docker` group, but {option}`services.docker` is not enabled.";
      }
    ];
